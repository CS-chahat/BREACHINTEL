...
def main():
    ...
    # Cap 60 breaches: matches model.py training data (not 200 — that was too
    # compressed, making 1 breach indistinguishable from 5 breaches)
    breach_norm   = log1p_norm(f.get("breach_count",    0),  60)   # cap 60 breaches
    password_norm = log1p_norm(f.get("password_leaks",  0),  40)   # cap 40 password leaks
    severity_norm = clamp(float(f.get("avg_severity",   0)) / 10.0)  # 0-10 → 0-1
    critical_norm = log1p_norm(f.get("critical_count",  0),  20)   # cap 20 critical
    recent_norm   = log1p_norm(f.get("recent_breaches", 0),  20)   # cap 20 recent
